...
const statusActions: Array<{ status: OrderStatus; label: string; color: string }> = [
  { status: "RECEIVED", label: "Recebido", color: "bg-blue-500 hover:bg-blue-600" },
  { status: "WASHING", label: "Em Lavagem", color: "bg-yellow-500 hover:bg-yellow-600" },
  { status: "READY", label: "Pronto", color: "bg-green-500 hover:bg-green-600" },
  { status: "DELIVERED", label: "Entregue", color: "bg-purple-500 hover:bg-purple-600" },
];

export function OrderDetailsModal({ event, open, onOpenChange }: OrderDetailsModalProps) {
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-[500px]">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm">{deliveryTypeLabels[order.deliveryType]}</span>
          </div>
              ...
              <div className="space-y-2">
                ...
                {order.specialInstructions && (
                  <div className="flex items-start gap-2">
                    <FileText className="mt-0.5 h-4 w-4 text-muted-foreground" />
                    <div>
                      <p className="text-xs text-muted-foreground">Instruções Especiais</p>
          ...
          <div>
            <p className="mb-2 text-sm font-medium text-muted-foreground">Alterar Status</p>
            <div className="flex flex-wrap gap-2">
              {statusActions.map((action) => (
                <Button
                  key={action.status}
                  size="sm"
                  disabled={order.status === action.status || updateStatus.isPending}
