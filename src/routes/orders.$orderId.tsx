...
import { useOrder, useUpdateOrderStatus, useDeleteOrder } from "@/hooks/use-orders";
...
function OrderDetailsPage() {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      await updateStatus.mutateAsync({ id: order.id, status: newStatus as OrderStatus });
      toast({
  ...
  const canDelete = order && order.status !== "DELIVERED" && order.status !== "CANCELLED";
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-xl">Excluir Pedido</DialogTitle>
            <DialogDescription className="text-center">
              Tem certeza que deseja excluir o pedido{" "}
              <span className="font-semibold text-foreground">{order.code}</span>? Esta
